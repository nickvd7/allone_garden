...
# ── Step 5: Hardened nginx + overload guard + fail2ban ───────────────────────
info "Applying hardened nginx + overload protection…"
GARDEN_DOMAIN="${DOMAIN}" GARDEN_EMAIL="${GARDEN_EMAIL:-}" \
  bash "${SCRIPT_DIR}/scripts/apply-nginx-hardening.sh" "$INSTALL_DIR"
success "Hardened nginx + overload guard applied"
